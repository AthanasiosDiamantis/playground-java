...
public class PredicateWithException {
    ...
    public static <T> Predicate<T> asUncheckedPrdicate(ExceptionalPredicate<T, Exception> predicate) {
        return t -> {
            try{
    ...
    public static void main(String[] args) {
        Predicate<Path> isEmptyFile = asUncheckedPrdicate( path -> Files.size(path) == 0);
